...
module HazardDetectionUnit(readRegister1, readRegister2, MemRead_1,
	...
	always @(readRegister1 or readRegister2 or Branch or RegtoPC
			 or MemRead_1 or writeRegister_I_1 or MemWrite
	...
	begin
		...
		if((Branch && RegWrite_1 && ((writeRegister_I_1 != 0 && !RegDst_1
									  && (readRegister1 == writeRegister_I_1
									   || readRegister2 == writeRegister_I_1))
							  	  || (writeRegister_R_1 != 0 && RegDst_1
		...
		// Bubble for JR if operand is calculated by RTYPE just before the JR
		|| (RegtoPC && RegWrite_1 && !MemtoReg_1
			&& ((writeRegister_R_1 != 0 && RegDst_1 && readRegister1 == writeRegister_R_1)
			 || (writeRegister_I_1 != 0 && !RegDst_1 && readRegister1 == writeRegister_I_1)))
		//  Bubble for JR if operand is calculated by LW just using it
		|| (RegtoPC && RegWrite_1 && MemtoReg_1
			&& writeRegister_I_1 != 0 && readRegister1 == writeRegister_I_1)
		// Second bubble for JR if operand is calculated by LW just before using it
		|| (RegtoPC && RegWrite_2 && MemtoReg_2
			&& writeRegister_2 != 0 && readRegister1 == writeRegister_2))
		begin
			PCWrite = 0;
			IF_ID_Write = 0;
			ControlSrc = 1;
		...
		// it is needed
		else if(MemRead_1 && !MemWrite && writeRegister_I_1 != 0
	...
	// Always need a bubble when jump instruction
	always @(Jump)
		if(Jump)
			IF_Flush = 1;
		else
	...
	// Always need a bubble when writing to PC with jr
	always @(RegtoPC)
		if(RegtoPC)
			IF_Flush = 1;
		else
